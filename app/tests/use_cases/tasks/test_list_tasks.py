...
def use_case(
    ...
    return ListTasksUseCase(repository, repository_user)
...
async def test_filter_tasks_by_title(
    ...
    tasks = await use_case.execute(user.id, TaskListFilter(title="Filter"))
    assert len(tasks) == 1
...
async def test_filter_tasks_by_description(
    ...
    tasks = await use_case.execute(user.id, TaskListFilter(description="description"))
