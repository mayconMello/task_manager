...
def use_case(
    ...
    return UpdateSubtaskUseCase(repository, repository_task)
...
async def test_update_subtask(
    ...
    update = SubtaskUpdate.model_validate(subtask)
    update.title = "Updated title"

    await use_case.execute(task.user_id, task.id, subtask.id, update)
...
async def test_update_subtask_with_invalid_task_id(
    ...
    with pytest.raises(ResourceNotFoundError):
        await use_case.execute(task.user_id, "invalid-task-id", subtask.id, update)
...
async def test_update_subtask_with_invalid_id(
    ...
    update = SubtaskUpdate.model_validate(subtask)
    with pytest.raises(ResourceNotFoundError):
        await use_case.execute(task.user_id, task.id, "invalid-subtask-id", update)
