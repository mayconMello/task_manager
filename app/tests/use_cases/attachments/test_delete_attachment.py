...
def use_case(
    ...
    return DeleteAttachmentUseCase(repository, repository_task)
...
async def test_delete_attachment(
    ...
    await use_case.execute(task.user_id, task.id, attachment.id)
...
async def test_delete_attachment_with_invalid_task_id(
    repository: InMemoryAttachmentRepository, use_case: DeleteAttachmentUseCase, task
):
    attachment = await repository.create(make_attachment())
    with pytest.raises(ResourceNotFoundError):
        await use_case.execute(task.user_id, "invalid-task-id", attachment.id)
...
async def test_delete_attachment_with_invalid_id(
    ...
    with pytest.raises(ResourceNotFoundError):
        await use_case.execute(task.user_id, task.id, "invalid-attachment-id")
