...
@pytest.mark.asyncio
async def test_e2e_get_subtask(
    client: AsyncClient,
    bearer_token: str,
    task: Task,
    subtask: Subtask,
    ...
    assert data["id"] == subtask.id.__str__()
    assert data["task_id"] == subtask.task_id.__str__()


@pytest.mark.asyncio
async def test_e2e_get_subtask_without_authentication(
    client: AsyncClient,
    task: Task,
    subtask: Subtask,
):
...
@pytest.mark.asyncio
async def test_e2e_get_subtask_with_invalid_id(client: AsyncClient, bearer_token: str, task: Task):
    response = await client.get(
