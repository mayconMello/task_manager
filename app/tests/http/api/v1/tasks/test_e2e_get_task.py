...
@pytest.mark.asyncio
async def test_e2e_get_task(client: AsyncClient, bearer_token: str, task: Task):
    ...
    assert data["id"] == task.id.__str__()


@pytest.mark.asyncio
async def test_e2e_get_task_without_authentication(client: AsyncClient, task: Task):
    response = await client.get(
        f"/api/v1/tasks/{task.id.__str__()}",
    )
